...
def train(model, dataloader, optimizer, loss_fn, device, dt):
    model.train()
    total_loss = 0
    ...
    for batch in tqdm(dataloader, desc="Training Progress", dynamic_ncols=True):
        ...
        u_t = batch.x[:, 0:1]        #Extracting - [u(t), v(t)]
...
def test(model, dataloader, loss_fn, device, dt):
    model.eval()
    total_loss = 0
...
def main(log_loss, total_epochs, batch_size):
    ...
    num_trajectories = 30
    selected_trajectories = get_trajectory_idx(outputs_u, num_trajectories, pca_components = 50)
    
    #Save trajectories for future use
    if num_trajectories != outputs_u.shape[0]:
        # np.save(f"Selected_indices_GNS_2D_Euler{num_trajectories}.npy", selected_trajectories)
        pass
    
    outputs_u_subselected = outputs_u[selected_trajectories, :, :, :, None]

    print("Indices selected: ", selected_trajectories)
    print(f"After subselection, outputs_u dataset shape: {outputs_u_subselected.shape}")

    Ns, Nt, Nx, Ny, Nv = outputs_u_subselected.shape
    ...
    for i in range(init_timestep+1, end_timestep):
        input_data_NN = np.vstack((input_data_NN, outputs_u_subselected[:,i,:,:,:]))
        output_data_NN = np.vstack((output_data_NN, outputs_u_subselected[:,i+1,:,:,:]))
