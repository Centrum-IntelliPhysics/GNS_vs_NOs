...
dataset = scipy.io.loadmat(base_path)
print(dataset.keys())

#For u-velocity field
#Consider 1st 1000 samples
outputs_u = jnp.array(dataset['u_train'])[:1000, ...]
...
result_dir = f"./FNO_AR_{num_trajectories}"
...
end_time = time.time()
print(f"Inference time for {u_pred.shape[0]} samples: {end_time-start_time} secs")
