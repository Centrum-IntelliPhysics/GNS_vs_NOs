...
Ns, Nt, Nx, Ny, Nv = outputs_uv.shape
...
input_node_feat_vector_dim = 8
input_edge_feat_vector_dim = 8
output_node_feat_vector_dim = 2
...
num_trajectories = 30
print(f"Inferring initialited from training on {num_trajectories}")
...
num_timesteps = Nt
dt = (1-0)/(Nt-1)
predicted_states = np.zeros_like(outputs_uv)

predicted_states[:, 0, :, :, :] = outputs_uv[:, 0, :, :, :]
...
print(f"Predictions: {predicted_states.shape}, output: {outputs_uv.shape}")

overall_rel_L2_err = np.linalg.norm(predicted_states - outputs_uv)/np.linalg.norm(outputs_uv)
print(f"Overall relative L2 error: {overall_rel_L2_err}")

#Computing overall relative L2 errors for u and v
rel_L2_err_u = np.linalg.norm(predicted_states[...,0] - outputs_uv[...,0])/np.linalg.norm(outputs_uv[...,0])
rel_L2_err_v = np.linalg.norm(predicted_states[...,1] - outputs_uv[...,1])/np.linalg.norm(outputs_uv[...,1])
...
if save:
    np.save(result_dir + "/uv_pred.npy", predicted_states)
    np.save(result_dir + "/uv_actual.npy", outputs_uv)