...
def run_inference(initial_uv, n_steps):
    uv_states = np.zeros_like(outputs_uv)  # List to store the states over time
    uv_states[:,0,:,:,:] = initial_uv
    
    # Initialize the previous state (this could be your u_0 and u_1, etc.)
    uv_curr = initial_uv  # Set the current state to the initial state
    
    for i in range(1, n_steps):
        uv_curr_in_FNO = create_input(uv_curr)    #(Ns, Nx, Ny, in_channels)
        uv_next_out_FNO = model_fn(best_params, uv_curr_in_FNO)   #(Ns, Nx, Ny, out_channels)
        uv_next = uv_next_out_FNO[:, :, :, :]
...
uv_start = outputs_uv[:, 0, :, :, :]
start_time = time.time()
uv_pred = run_inference(uv_start, n_steps=Nt)
end_time = time.time()
print(f"Inference complete for {uv_pred.shape[0]} samples: {end_time-start_time} secs")
print("Post inference, shapes...")
print(f"Predicted: {uv_pred.shape}, Outputs: {outputs_uv.shape}")

#Compute relative L2 error between predicted and outputs
rel_l2_err = np.linalg.norm(uv_pred - outputs_uv)/np.linalg.norm(outputs_uv)
print(f"Overall relative L2 error: {rel_l2_err}")

#Compute relative L2 errors separately for u and v velocity fields
rel_l2_err_u = np.linalg.norm(uv_pred[...,0] - outputs_uv[...,0])/np.linalg.norm(outputs_uv[...,0])
rel_l2_err_v = np.linalg.norm(uv_pred[...,1] - outputs_uv[...,1])/np.linalg.norm(outputs_uv[...,1])
...
save = False
if save:
    np.save(result_dir + "/uv_pred.npy", uv_pred)
