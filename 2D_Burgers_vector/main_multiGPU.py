...
def train(model, dataloader, optimizer, loss_fn, device, dt = 1e-2):
    ...
    for batch in tqdm(dataloader, desc="Training Progress", dynamic_ncols=True):
        ...
        #Predict the time derivative at state i - [du/dt, dv/dt]
        grad_pred = model(batch.x, batch.edge_index, batch.edge_attr)
        
        #Roll one-step forward using an Euler integrator
        u_t = batch.x[:, 0:2]        #Extracting - [u(t), v(t)]
        
        # #Euler step
        u_t_plus_1 = u_t + grad_pred * dt      #Compute [u(t+1), v(t+1)]
...
def main(log_loss, total_epochs, batch_size):
    ...
    initial_u = dataset["initial_field_u"].cpu().detach().numpy()
    outputs_u = dataset["output_field_u"].cpu().detach().numpy()

    #For v-velocity field
    initial_v = dataset["initial_field_v"].cpu().detach().numpy()
    outputs_v = dataset["output_field_v"].cpu().detach().numpy()

    #Check if the IVP can be cast into a Cauchy problem
    #This means inputs = outputs[t=0]
    max_err_u = np.max(np.abs(outputs_u[:, 0, :, :] - initial_u))
    max_err_v = np.max(np.abs(outputs_v[:, 0, :, :] - initial_v))

    print("Checking if its a Cauchy Problem")
    print(f"Max error in inputs and outputs[t=0] for u-field: {max_err_u}")
    print(f"Max error in inputs and outputs[t=0] for v-field: {max_err_v}")

    #Concatenate the initial velocity fields - u and v
    outputs_uv = np.concatenate([outputs_u[..., None], outputs_v[..., None]], axis=-1)

    #Inspecting shapes before subselection of training trajectories
    print(f"Inspecting shape of the outputs_uv dataset before subselection: {outputs_uv.shape}")
    ...
    print(f"After subselection, outputs_uv dataset shape: {outputs_uv_subselected.shape}")
    ...
    for i in range(init_timestep+1, end_timestep):
        input_data_NN = np.vstack((input_data_NN, outputs_uv_subselected[:,i,:,:]))
        output_data_NN = np.vstack((output_data_NN, outputs_uv_subselected[:,i+1,:,:]))
    ...
    output_node_feat_vector_dim = train_dataset[0].y.shape[-1]
    ...
    file_name_params = f"best_model_params_{num_trajectories}.pth"
    ...
    if rank==0:
        ...
        plt.legend(loc = "best")
        plt.grid()
        # plt.savefig(result_dir + f"/loss_curves_{num_trajectories}.jpeg", dpi=200)
