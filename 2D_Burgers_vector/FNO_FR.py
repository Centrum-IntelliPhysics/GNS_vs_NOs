...
print(f"After subselection, shape: {outputs_uv_subselected.shape}")

#Free up memory
del outputs_uv, dataset

Ns, Nt, Nx, Ny, Nv = outputs_uv_subselected.shape
print(f"Ns: {Ns}, Nt: {Nt}, Nx: {Nx}, Ny: {Ny}, Nv: {Nv}")

#Assigning inputs and outputs
inputs = outputs_uv_subselected[:, 0, :, :, :]     #(Ns, Nx, Ny, Nv)
outputs = outputs_uv_subselected[:, :, :, :, :]    #(Ns, Nt, Nx, Ny, Nv)
...
uv_init = outputs_uv[:, 0, :, :, :]     #(Ns, Nx, Ny, Nv)
...
for i in range(0, Ns, BATCH):
    u0_batch = uv_init[i:i+BATCH]   # (B, Nx, Ny, Nv)
...
rel_l2_err = np.linalg.norm(pred_FNO - outputs_uv)/np.linalg.norm(outputs_uv)
print(f"Overall relative L2 error with {num_trajectories}: {rel_l2_err}")


#Compute relative L2 error for individual fields
rel_l2_err_u = np.linalg.norm(pred_FNO[...,0] - outputs_uv[...,0])/np.linalg.norm(outputs_uv[...,0])
rel_l2_err_v = np.linalg.norm(pred_FNO[...,1] - outputs_uv[...,1])/np.linalg.norm(outputs_uv[...,1])
...
if save:
    np.save(result_dir + "/uv_pred.npy", pred_FNO)
