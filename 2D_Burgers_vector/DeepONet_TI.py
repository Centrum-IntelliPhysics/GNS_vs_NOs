...
@jax.jit
def loss_fn(params_u, params_v, branch_inputs, trunk_inputs, gt_outputs, dt):
    
    #U-velocity field
    u_curr = branch_inputs[...,0]
    u_next = gt_outputs[...,0]
    u_pred_next = rk4_update(model_fn_u, params_u, u_curr, trunk_inputs, dt)
    
    #V-velocity field
    v_curr = branch_inputs[...,1]
    v_next = gt_outputs[...,1]
    v_pred_next = rk4_update(model_fn_v, params_v, v_curr, trunk_inputs, dt)
...
@jax.jit
def update(params_u, params_v, branch_inputs, trunk_inputs, gt_outputs, opt_state_u, opt_state_v, dt):
...
u_pred = run_inference(model_fn_u, best_params_u, u_start, trunk_inputs_test, 
                       n_steps=Nt, method="AB", dt=dt_val)
print("Inference complete for u-velocity field")

v_pred = run_inference(model_fn_v, best_params_v, v_start, trunk_inputs_test, 
                       n_steps=Nt, method="AB", dt=dt_val)
print("Inference complete for v-velocity field")
end_time = time.time()
print(f"Overal inference time for u and v velocity fields: {end_time-start_time} secs")
