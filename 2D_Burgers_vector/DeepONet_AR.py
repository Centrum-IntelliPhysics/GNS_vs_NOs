...
print(f"Shapes of output u and v velocity fields: u = {outputs_u.shape}, v = {outputs_v.shape}")

#Concatenating outputs
outputs_uv = jnp.stack([outputs_u, outputs_v], axis=-1)
print(f"After concatenating u and v, shape: {outputs_uv.shape}")
...
print(f"After subselection: {outputs_uv_subselected.shape}")
...
def loss_fn(params_u, params_v, branch_inputs, trunk_inputs, gt_outputs):
    ...
    u_next = gt_outputs[...,0]
    u_pred_next = model_fn_u(params_u, u_curr, trunk_inputs)
    
    #V-velocity field
    v_curr = branch_inputs[...,1]
    v_next = gt_outputs[...,1]
    v_pred_next = model_fn_v(params_v, v_curr, trunk_inputs)
...
def inference(model_fn_inf, params_inf, u_curr, trunk_inputs_test):
    u_next = model_fn_inf(params_inf, u_curr, trunk_inputs_test)
    return u_next

def run_inference(model_fn_inf, params_inf, initial_u, trunk_inputs_test, n_steps):
    
    u_states = np.zeros(shape = (Ns, Nt, Nx, Ny))
    ...
    for i in range(1, n_steps):
        ...
        u_next = inference(model_fn_inf, params_inf, u_curr, trunk_inputs_test)
...
u_pred = run_inference(model_fn_u, best_params_u, u_start, trunk_inputs_test, n_steps=Nt)
print("Inference complete for u-velocity field")

v_pred = run_inference(model_fn_v, best_params_v, v_start, trunk_inputs_test, n_steps=Nt)
