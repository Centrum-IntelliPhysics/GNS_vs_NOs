...
input_node_feat_vector_dim = 9
input_edge_feat_vector_dim = 9
output_node_feat_vector_dim = 3
...
num_trajectories = 30
print(f"Inference initialized from training on {num_trajectories}")
...
num_timesteps = Nt
dt = (1-0)/(Nt-1)
predicted_states = np.zeros_like(outputs_uvh)

predicted_states[:, 0, :, :, :] = outputs_uvh[:, 0, :, :, :]
...
print(f"Predictions: {predicted_states.shape}, output: {outputs_uvh.shape}")
print(f"Inference time = {end_time - start_time} secs")

overall_rel_L2_err = np.linalg.norm(predicted_states - outputs_uvh)/np.linalg.norm(outputs_uvh)
print(f"Overall relative L2 error: {overall_rel_L2_err}")

#Compute relative L2 errors in u, v, and h separately
rel_l2_err_u = np.linalg.norm(predicted_states[...,0] - outputs_uvh[...,0])/np.linalg.norm(outputs_uvh[...,0])
rel_l2_err_v = np.linalg.norm(predicted_states[...,1] - outputs_uvh[...,1])/np.linalg.norm(outputs_uvh[...,1])
rel_l2_err_h = np.linalg.norm(predicted_states[...,2] - outputs_uvh[...,2])/np.linalg.norm(outputs_uvh[...,2])

print("Relative L2 errors for different fields...")
print(f"u-velocity field: {rel_l2_err_u}")
print(f"v-velocity field: {rel_l2_err_v}")
print(f"h-velocity field: {rel_l2_err_h}")

#Save predictions and outputs
save = False

if save:
    print(f"Saving result arrays for {num_trajectories} training trajectories..")
    np.save(result_dir + f"/u_pred_{num_trajectories}_bs1.npy", predicted_states)

    if num_trajectories == 30:
        np.save(result_dir + f"/u_actual_{num_trajectories}.npy", outputs_uvh)