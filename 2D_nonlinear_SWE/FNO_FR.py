...
inputs = outputs_uvh_subselected[:, 0, :, :, :]     #(Ns, Nx, Ny, Nv)
outputs = outputs_uvh_subselected[:, :, :, :, :]    #(Ns, Nt, Nx, Ny, Nv)

# Create coordinate grids
xspan = np.linspace(0, 1, Nx)  # spatial domain - x
yspan = np.linspace(0, 1, Ny)  # spatial domain - y
tspan = np.linspace(0, 1, Nt)  # temporal domain - t
...
#Free up some memory
del inputs_tiled, T_tiled, X_tiled, Y_tiled, inputs
...
# print(outputs_u.shape, outputs_v.shape, outputs_h.shape)
...
# uvh_init = outputs_uvh[:, 0, :, :, :]     #(Ns, Nx, Ny, Nv)
...
#     """
#     u0: (B, Nx, Ny, Nv)    initial condition
#     coords: (Nt, Nx, Ny, 3)  shared (t,x,y) grid
#     returns: (B, Nt, Nx, Ny, Nv+3)  predicted solution
#     """
#     B, Nx, Ny, _ = u0.shape
#     Nt = coords.shape[0]

#     # Broadcast u0 along time
#     u0_b = jnp.broadcast_to(u0[:, None, ...], (B, Nt, Nx, Ny, Nv))  # (B, Nt, Nx, Ny, Nv)

#     # Add batch axis to coords
#     coords_b = jnp.broadcast_to(coords[None, ...], (B, Nt, Nx, Ny, 3))  # (B, Nt, Nx, Ny, 3)

#     # Concatenate along channel axis
#     inputs = jnp.concatenate([u0_b, coords_b], axis=-1)  # (B, Nt, Nx, Ny, Nv+3)
...
#     u0_batch = uvh_init[i:i+BATCH]   # (B, Nx, Ny, Nv)
#     pred = fno_forward(best_params, u0_batch, coords)
#     preds.append(pred)
# pred_FNO = jnp.concatenate(preds, axis=0)  # (Ns, Nt, Nx, Ny, 2)
...
# rel_l2_err = np.linalg.norm(pred_FNO - outputs_uvh)/np.linalg.norm(outputs_uvh)
# print(f"Overall relative L2 error with {num_trajectories}: {rel_l2_err}")

# #Compute relative L2 error for individual fields
# rel_l2_err_u = np.linalg.norm(pred_FNO[...,0] - outputs_uvh[...,0])/np.linalg.norm(outputs_uvh[...,0])
# rel_l2_err_v = np.linalg.norm(pred_FNO[...,1] - outputs_uvh[...,1])/np.linalg.norm(outputs_uvh[...,1])
# rel_l2_err_h = np.linalg.norm(pred_FNO[...,2] - outputs_uvh[...,2])/np.linalg.norm(outputs_uvh[...,2])
...
# save = True
# if save:
#     np.save(result_dir + f"/uvh_pred_{num_trajectories}.npy", pred_FNO)
